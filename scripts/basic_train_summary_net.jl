...
for e=1:n_epochs# epoch loop
    ...
    if(mod(e,plot_every)==0) 
	    ...
	    for (latent_test_x, latent_test_y) in [[X_train,Y_train], [X_test, Y_test]]
		    ...
		    for i in 1:plot_len
			    ...
			    subplot(1,2,1); imshow(y_plot, vmin=-a,vmax=a,aspect=0.5,interpolation="none", cmap="seismic")
			    ...
			    subplot(plot_len,num_cols,(i-1)*num_cols+1); imshow(y_plot, vmin=-a,vmax=a,interpolation="none", cmap="seismic")
				colorbar(fraction=0.046, pad=0.04);title(L"$y$")
			
			    subplot(plot_len,num_cols,(i-1)*num_cols+2); imshow(X_post[:,:,1,1], vmin=vmin,vmax=vmax, interpolation="none", cmap="cet_rainbow4")
				axis("off");  colorbar(fraction=0.046, pad=0.04);title("Posterior sample")

				subplot(plot_len,num_cols,(i-1)*num_cols+3); imshow(X_post[:,:,1,2], vmin=vmin,vmax=vmax, interpolation="none", cmap="cet_rainbow4")
				axis("off");  colorbar(fraction=0.046, pad=0.04);title("Posterior sample")

				x_plot = x[:,:,1,1]
				subplot(plot_len,num_cols,(i-1)*num_cols+4); imshow(x_plot,  vmin=vmin,vmax=vmax,  interpolation="none", cmap="cet_rainbow4")
				axis("off"); title(L"$\mathbf{x_{gt}}$") ; colorbar(fraction=0.046, pad=0.04)

				subplot(plot_len,num_cols,(i-1)*num_cols+5); imshow(X_post_mean[:,:,1,1] ,  vmin=vmin,vmax=vmax,  interpolation="none", cmap="cet_rainbow4")
			...
			tight_layout()
		    fig_name = @strdict  n_c unet_lev nx sum_net clipnorm_val noise_lev_x n_train  e lr n_hidden L K batch_size
		    if k == 1
    ...
    if(mod(e,save_every)==0) 
    	...
    	if sum_net
			...
			global save_dict = @strdict  n_c  unet_lev unet_model n_in nx sum_net clipnorm_val n_train e noise_lev_x lr n_hidden L K Params loss logdet_train l2_cm ssim loss_test logdet_test l2_cm_test ssim_test batch_size; 
		else 
	        G_save = deepcopy(G);
			Params = get_params(G_save) |> cpu;
			global save_dict = @strdict  n_c unet_lev  n_in nx sum_net clipnorm_val n_train e noise_lev_x lr n_hidden L K Params loss logdet_train l2_cm ssim loss_test logdet_test l2_cm_test ssim_test batch_size; 
		end;
